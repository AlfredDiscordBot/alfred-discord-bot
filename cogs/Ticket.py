    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.emoji.name == chr(127915):
            ...
            if payload.guild_id not in self.client.config["ticket"]:
                return
            if not self.client.get_channel(
                self.client.config["ticket"][payload.guild_id][0]
            ):
                del self.client.config["ticket"][payload.guild_id]
                return
            if payload.channel_id != self.client.config["ticket"][payload.guild_id][0]:
                return
            ...
            if msg != self.client.config["ticket"][payload.guild_id][1]:
                return
            await ms.remove_reaction(payload.emoji, payload.member)
            mess = await channel.send(
                embed=ef.cembed(
                    description=f"Creating Ticket for {payload.member.name}",
                    color=self.client.re[8],
                )
            )

            th = await channel.create_thread(
                name=f"Ticket - {payload.member.name} {payload.member.id}",
                reason=f"Ticket - {payload.member.name}",
