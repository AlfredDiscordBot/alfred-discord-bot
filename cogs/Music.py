...
class Music(commands.Cog):
    ...
    async def pause(self, ctx):
        ...
        if mem.count(user.id) > 0:
            ...
            voice.pause()
            url = self.client.queue_song[str(ctx.guild.id)][
                self.client.re[3][str(ctx.guild.id)]
            ]
    ...
    async def addto(
        self,
        inter,
        mode=ef.defa(choices=["queue", "playlist", "show", "clear"]),
        from_user: nextcord.User = "-",
    ):
        ...
        if mode == "queue":
            if from_user.id in list(self.client.da.keys()):
                self.client.queue_song[str(inter.guild.id)] += self.client.da[
                    from_user.id
                ]
        ...
        if user.voice and user.voice.channel:
            ...
            try:
                ...
                if mem.count(user.id) > 0:
                    ...
                    song = self.client.queue_song[str(ctx.guild.id)][
                        self.client.re[3][str(ctx.guild.id)]
                    ]
        ...
        try:
            if ef.check_voice(ctx):
                ...
                if self.client.re[3][str(ctx.guild.id)] == -1:
                    self.client.re[3][str(ctx.guild.id)] = (
                        len(self.client.queue_song.get(str(ctx.guild.id), [])) - 1
                    )
                song = self.client.queue_song[str(ctx.guild.id)][
                    self.client.re[3][str(ctx.guild.id)]
                ]
    ...
    def repeat(self, ctx, voice):
        ...
        if self.client.re[7].get(ctx.guild.id, -1) == 1 and not voice.is_playing():
            self.client.re[3][str(ctx.guild.id)] += 1
            index = self.client.re[3].get(str(ctx.guild.id), 0)
            if self.client.re[3][str(ctx.guild.id)] >= len(
                self.client.queue_song[str(ctx.guild.id)]
            ):
                self.client.re[3][str(ctx.guild.id)] = 0
        if (
        ...
        try:
            if ef.check_voice(ctx):
                ...
                if ind.isnumeric():
                    ...
                    self.client.re[3][str(ctx.guild.id)] = len(
                        self.client.queue_song[str(ctx.guild.id)]
                    )
                    songs = self.client.queue_song[str(ctx.guild.id)]
        ...
        try:
            if ef.check_voice(ctx):
                ...
                if self.client.re[3][str(ctx.guild.id)] >= len(
                    ...
                    self.client.re[3][str(ctx.guild.id)] = (
                        len(self.client.queue_song[str(ctx.guild.id)]) - 1
                    )
                    await ctx.send(
                        embed=ef.cembed(
                            title="Last song",
