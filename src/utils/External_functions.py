...
def activities(client):
    ...
    all_activities = [
        ...
        nextcord.Activity(
            type=nextcord.ActivityType.watching, name="Nextcord People do their magic"
        ),
        nextcord.Activity(
            type=nextcord.ActivityType.listening, name="Wayne Enterprises"
        ),
        nextcord.Activity(
...
def imdb_embed(movie="", re={8: 5160}):
    ...
    try:
        ...
        embed = cembed(title=mov["title"], description=plot, color=re[8], image=image)
        n = 0
        for i in di:
            n += 1
                ...
                for rows in tables.find_all("li"):
                    if a := rows.a:
                        self.CATEGORIES.update(
                            {a.get_text(): self.BASE_URL + a["href"]}
                        )
