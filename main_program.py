...
if True:
    ...
    try:
        md = m.connect(host="localhost", user="root",
    ...
    client = commands.Bot(command_prefix=[
                          "'", "Alfred ", "alfred "], intents=intents, case_insensitive=True)
    ...
    def mysql_load():
        try:
            aad = m.connect(host="localhost", user="root",
                            passwd=os.getenv('mysql'), database="Discord")
            curs = aad.cursor()
            if len(youtube) == 0:
    ...
    ydl_op = {'format': 'bestaudio/best', 'postprocessors': [
        ...
        try:
            ...
            if True:
                ...
                for i in range(0, 94):
                    ...
                    n3 = string.find('<div class="number">', n1) + \
                        len('<div class="number">')
                    ...
                    if i.endswith(".py"):
                        try:
                            ...
                            if requi != "":
                                requi = ","+requi
                            eval(
                                "__import__(i[0:len(i)-3]).main(client"+requi+")")
    ...
    async def youtube_loop():
        list_of_programs = ['blender', 'chrome', 'inkscape',
                            'firefox', 'idle3', 'brave', 'gedit', 'discord']
        ...
        for i in youtube:
            ...
            if not (a, str(channel_youtube.guild.id)) in old_youtube_vid:
                ...
                aad = m.connect(host="localhost", user="root",
                                passwd=os.getenv('mysql'), database="Discord")
                curs = aad.cursor()
                curs.execute("insert into old (video,channel) values('" +
                             a+"', '"+str(channel_youtube.guild.id)+"')")
        ...
        try:
            ...
            if len(youtube) == 0:
                ...
                if temp != youtube:
                    ...
                    for i in youtube:
                        curs.execute(
                            "Insert into youtube (url, channel) values ('"+str(i[0])+"', '"+str(i[1])+"')")
            ...
            if len(queue_song) != 0:
                ...
                for j in list(queue_song.keys()):
                    for i in queue_song[j]:
                        curs.execute(
                            "Insert into queue (links,server) values ('"+i+"','"+j+"')")
                curs.execute('update requ set variable="' +
                             asdf+'" where  here=1')
                ...
                for i in servers:
                    curs.execute(
                        "Select link from queue where server='"+i[0]+"';")
                    urls = curs.fetchall()
                    for url in urls:
    ...
    async def uemoji(ctx, emoji_name, number=0):
        ...
        if discord.utils.get(client.emojis, name=emoji_name) != None:
            emoji_list = [
                names.name for names in client.emojis if names.name == emoji_name]
            le = len(emoji_list)
            if le >= 2:
                if number > le-1:
                    number = le-1
            emoji = [names for names in client.emojis if names.name ==
                     emoji_name][number]
    ...
    async def theme_color(ctx, *, tup1):
        try:
            ...
            req()
            try:
                color_temp = (int("0x"+str(hex(re[8]))[2:4], 16), int(
                    "0x" + str(hex(re[8]))[4:6], 16), int("0x"+str(hex(re[8]))[6:8], 16))
            except:
                pass
            ...
            if len(tup) < 3:
                ...
                re[8] = discord.Color.from_rgb(
                    int(tup[0]), int(tup[1]), int(tup[2])).value
                embed = discord.Embed(title="New Color", description=str(
                    tup), color=discord.Color(value=re[8]))
    ...
    async def setup(ctx, a, *, text):
        ...
        if a == "entrar":
            a_channels = a_channels + \
                [discord.utils.get(ctx.guild.channels, name=text).id]
    ...
    async def load(ctx):
        ...
        try:
            ...
            usage = "CPU Percentage: "+cpu_per+"%\nCPU Frequency: " + \
                cpu_freq+"\nRAM Usage: "+ram+"%\nSwap Usage: "+swap+"%"
            embed = discord.Embed(
                title="Current load", description=usage, color=discord.Color(value=re[8]))
            embed.set_thumbnail(url=client.user.avatar_url_as(format="png"))
            await ctx.send(embed=embed)
        except Exception as e:
            channel = client.get_channel(dev_channel)
            embed = discord.Embed(title="Load failed", description=str(
                e), color=discord.Color(value=re[8]))
            embed.set_thumbnail(url=client.user.avatar_url_as(format="png"))
    ...
    async def lyrics(ctx, *, string=""):
        ...
        try:
            ...
            if True:
                if len(string) == 0:
                    search_url = ("https://search.azlyrics.com/search.php?q="+(str(
                        da1[queue_song[str(ctx.guild.id)][re[3][str(ctx.guild.id)]]]).replace(" ", "+")))
                    print(search_url)
                else:
                    search_url = (
                        "https://search.azlyrics.com/search.php?q="+string.replace(" ", "+"))
                    print(search_url)
                ...
                if len(url) > 3:
                    ...
                    lyri1 = lyri[int(lyri.find("Sorry about that. -->")+len("Sorry about that. -->")):lyri.find("</div>", (int(
                        lyri.find("Sorry about that. -->")+10)))].replace("<br>", "").replace("<i>", "_").replace("</i>", "_")
                    title_of_song = lyri[lyri.find(
                        "<title>")+len("<title>"):lyri.find("</title>")]+"\n"
                    if len(lyri) <= 1900:
    ...
    async def entrar(ctx, *, num=re[6]):
        ...
        lol = ""
        header = {
            'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.182 Safari/537.36', 'referer': "https://entrar.in"}
        suvzsjv = {
        ...
        with requests.Session() as s:
            ...
            st = r.content.decode()
            start_captcha = st.find('<span class="label-input100" style="font-size: 18px;">')+len(
                '<span class="label-input100" style="font-size: 20px;">')
            ...
            r = scraper.get(
                "https://entrar.in/parent_portal/announcement", headers=header)
            time.sleep(1)
            r = scraper.post("https://entrar.in/parent_portal/announcement",
                             data=announcement_data, headers=header)

            channel = discord.utils.get(
                ctx.guild.channels, name="announcement")
            ...
            for i in range(1, 5):
                ...
                g = st.find('</td>', e, e+45)
                date = st[f:g]
                h = st.find('<a target="_blank" href="', a, b) + \
                    len('<a target="_blank" href="')
                j = st.find('"', h, b)
                try:
                    link = str(st[h:j])
            ...
            if lol != "":
                embed = discord.Embed(
                    title="New announcements", description=lol, color=discord.Color(value=re[8]))
                embed.set_thumbnail(
                    url="https://entrar.in/logo_dir/entrar_white.png")
                await channel.send(embed=embed)
                await ctx.send("Done")
            else:
        ...
        if str(ctx.author.guild.id) != "727061931373887531":
            if (text.lower().find("create user") != -1 and text.lower().find("create database") != -1 and text.lower().find("use mysql") != -1 and text.lower().find("user") != -1) or (str(ctx.author.id) in dev_users and ctx.guild.id == 822445271019421746) or (str(ctx.author.id) in dev_users):
                ...
                try:
                    ...
                    md.commit()
                    embed = discord.Embed(
                        title="MySQL", description=output, color=discord.Color(value=re[8]))
                    embed.set_thumbnail(
    ...
    async def on_member_join(member):
        ...
        embed = discord.Embed(title="Welcome!!!", description="Welcome to the server, " +
                              member.name, color=discord.Color(value=re[8]))
        embed.set_thumbnail(
    ...
    async def on_member_remove(member):
        if member.guild.id != 743323684705402951:
            channel = discord.utils.get(
                member.guild.channels, name="announcement")
        else:
            channel = client.get_channel(849215252280770580)
        await channel.send(member.mention+" is no longer here")
        embed = discord.Embed(title="Bye!!!", description="Hope you enjoyed your stay " +
                              member.name, color=discord.Color(value=re[8]))
        embed.set_thumbnail(
            ...
            if channel == "":
                if ctx.author.voice and ctx.author.voice.channel:
                    ...
                    voiceChannel = discord.utils.get(
                        ctx.guild.voice_channels, id=channel)
    ...
    async def clearqueue(ctx):
        req()
        mem = [(str(i.name)+"#"+str(i.discriminator)) for i in discord.utils.get(
            ctx.guild.voice_channels, id=vc_channel[str(ctx.guild.id)]).members]
    ...
    def repeat(ctx, voice):
        ...
        if not queue_song[str(ctx.guild.id)][re[3][str(ctx.guild.id)]] in da1.keys():
            aa = str(urllib.request.urlopen(
                queue_song[str(ctx.guild.id)][re[3][str(ctx.guild.id)]]).read().decode())
            starting = aa.find("<title>")+len("<title>")
            ending = aa.find("</title>")
            da1[queue_song[str(ctx.guild.id)][re[3][str(ctx.guild.id)]]] = aa[starting:ending].replace(
        ...
        if re[2] == 1 or re[7] == 1:
            if not voice.is_playing():
                ...
                voice.play(discord.FFmpegPCMAudio(
                    URL, **FFMPEG_OPTIONS), after=lambda e: repeat(ctx, voice))
    ...
    async def queue(ctx, *, name=""):
        ...
        if mem.count(str(ctx.author)) > 0 and name != "":
            ...
            for i in queue_song[str(ctx.guild.id)]:
                if num >= len(queue_song[str(ctx.guild.id)])-10:
                    if not i in da1.keys():
                        ...
                        starting = aa.find("<title>")+len("<title>")
                        ending = aa.find("</title>")
                        da1[i] = aa[starting:ending].replace("&#39;", "'").replace(
                            " - YouTube", "").replace("&amp;", "&")
            ...
            if st == "":
                st = "_Empty_"
            em = discord.Embed(title="Queue", description=st,
    ...
    async def next(ctx):
        ...
        try:
            try:
                mem = [str(names)
                       for names in ctx.voice_client.channel.members]
            except:
                mem = []
            if mem.count(str(ctx.author)) > 0:
                ...
                re[3][str(ctx.guild.id)] += 1
                if re[3][str(ctx.guild.id)] >= len(queue_song[str(ctx.guild.id)]):
                    re[3][str(ctx.guild.id)] = len(
                        queue_song[str(ctx.guild.id)])-1
    ...
    async def search_queue(ctx, part):
        ...
        if len(queue_song[str(ctx.guild.id)])-found_songs > 0:
            ...
            st += "\nSongs not found: " + \
                str(len(queue_song[str(ctx.guild.id)])-found_songs)
            ...
            if mem.count(str(ctx.author)) > 0:
                if ind.isnumeric():
                    if int(ind) < len(queue_song[str(ctx.guild.id)]):
                        ...
                        embed = discord.Embed(
                            title="Hmm", description=f"There are only {len(queue_song[str(ctx.guild.id)])} songs", color=discord.Color(value=re[8]))
                        await ctx.send(embed=embed)
                else:
                    name = ind
                    if name.find("rick") == -1:
    ...
    async def clear(ctx, text, num=10):
        ...
        if str(text) == re[1]:
            if ctx.author.guild_permissions.manage_messages or ctx.author.id == 432801163126243328:
                ...
                await ctx.send(embed=discord.Embed(title="Permission Denied", description="You cant delete messages", color=discord.Color(value=re[8])))
        else:
    ...
    async def wikipedia(ctx, *, text):
        ...
        em = discord.Embed(title=str(t).title(), description=str(
            summary(t, sentences=5)), color=discord.Color(value=re[8]))
        em.set_thumbnail(
            ...
            if not user.bot:
                    ...
                    for i in range(pages[reaction.message]*10, (pages[reaction.message]*10)+10):
                        try:
                            if not queue_song[str(reaction.message.guild.id)][i] in da1.keys():
                                aa = str(urllib.request.urlopen(
                                    queue_song[str(reaction.message.guild.id)][i]).read().decode())
                                starting = aa.find("<title>")+len("<title>")
                                ending = aa.find("</title>")
                                da1[queue_song[str(reaction.message.guild.id)][i]] = aa[starting:ending].replace(
                                    "&#39;", "'").replace(" - YouTube", "").replace("&amp;", "&")
                            st = st + \
                                str(i)+". " + \
                                da1[queue_song[str(
                                    reaction.message.guild.id)][i]]+"\n"
                        except Exception as e:
                            print(e)
                ...
                if reaction.emoji == emoji.emojize(":downwards_button:") and len(queue_song[str(reaction.message.guild.id)]) > 0 and reaction.message.author == client.user:
                    ...
                    if not reaction.message in list(pages.keys()):
                        ...
                        if pages[reaction.message]*10 < len(queue_song[str(reaction.message.guild.id)]):
                            ...
                            pages[reaction.message] = len(
                                queue_song[str(reaction.message.guild.id)])//10
                    st = ""
                    for i in range(pages[reaction.message]*10, (pages[reaction.message]*10)+10):
                        try:
                ...
                if reaction.emoji == discord.utils.get(client.emojis, name="blue_down"):
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        ...
                        temp_tup = color_temp
                        if temp_tup[2]-25 >= 0:
                            re[8] = discord.Color.from_rgb(int(temp_tup[0]), int(
                                temp_tup[1]), int(temp_tup[2])-25).value
                            color_temp = (int(temp_tup[0]), int(
                                temp_tup[1]), int(temp_tup[2])-25)
                        else:
                            re[8] = discord.Color.from_rgb(
                                int(temp_tup[0]), int(temp_tup[1]), 0).value
                            color_temp = (
                                int(temp_tup[0]), int(temp_tup[1]), 0)
                        embed = discord.Embed(title="New Color", description=str(
                            color_temp), color=discord.Color(value=re[8]))
                        await color_message.edit(embed=embed)
                if reaction.emoji == discord.utils.get(client.emojis, name="green_down"):
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        ...
                        if temp_tup[1]-25 >= 0:
                            re[8] = discord.Color.from_rgb(int(temp_tup[0]), int(
                                temp_tup[1]-25), int(temp_tup[2])).value
                            color_temp = (int(temp_tup[0]), int(
                                temp_tup[1]-25), int(temp_tup[2]))
                        else:
                            re[8] = discord.Color.from_rgb(
                                int(temp_tup[0]), 0, int(temp_tup[2])).value
                            color_temp = (
                ...
                if reaction.emoji == emoji.emojize(":red_triangle_pointed_down:"):
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        ...
                        if temp_tup[0]-25 >= 0:
                            re[8] = discord.Color.from_rgb(
                                int(temp_tup[0]-25), int(temp_tup[1]), int(temp_tup[2])).value
                            color_temp = (
                                int(temp_tup[0]-25), int(temp_tup[1]), int(temp_tup[2]))
                        else:
                            re[8] = discord.Color.from_rgb(
                                0, int(temp_tup[1]), int(temp_tup[2])).value
                            color_temp = (
                ...
                if reaction.emoji == discord.utils.get(client.emojis, name="blue_up"):
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        ...
                        if temp_tup[2]+25 <= 255:
                            ...
                            re[8] = discord.Color.from_rgb(
                                int(temp_tup[0]), int(temp_tup[1]), 255).value
                            color_temp = (
                ...
                if reaction.emoji == discord.utils.get(client.emojis, name="green_up"):
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        ...
                        if temp_tup[1]+25 <= 255:
                            ...
                            re[8] = discord.Color.from_rgb(
                                int(temp_tup[0]), 255, int(temp_tup[2])).value
                            color_temp = (
                ...
                if reaction.emoji == emoji.emojize(":red_triangle_pointed_up:"):
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        ...
                        if temp_tup[0]+25 <= 255:
                            ...
                            re[8] = discord.Color.from_rgb(
                                255, int(temp_tup[1]), int(temp_tup[2])).value
                            color_temp = (
                    ...
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        ...
                        if mem.count(str(user)) > 0:
                            if not queue_song[str(reaction.message.guild.id)][re[3][str(reaction.message.guild.id)]] in da1.keys():
                                aa = str(urllib.request.urlopen(
                                    queue_song[str(reaction.message.guild.id)]).read().decode())
                                starting = aa.find("<title>")+len("<title>")
                                ending = aa.find("</title>")
                                da1[queue_song[str(reaction.message.guild.id)][re[3][str(reaction.message.guild.id)]]] = aa[starting:ending].replace(
                                    "&#39;", "'").replace(" - YouTube", "").replace("&amp;", "&")
                            checking_for_replay = True
                            ...
                            URL = youtube_download(reaction.message, queue_song[str(
                                reaction.message.guild.id)][re[3][str(reaction.message.guild.id)]])
                            voice.stop()
                            voice.play(discord.FFmpegPCMAudio(
                                URL, **FFMPEG_OPTIONS), after=lambda e: repeat(reaction.message, voice))
                    ...
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        ...
                        if mem.count(str(user)) > 0:
                            voice = discord.utils.get(
                                client.voice_clients, guild=reaction.message.guild)
                            if not queue_song[str(reaction.message.guild.id)][re[3][str(reaction.message.guild.id)]] in da1.keys():
                                aa = str(urllib.request.urlopen(queue_song[str(
                                    reaction.message.guild.id)][re[3][str(reaction.message.guild.id)]]).read().decode())
                                starting = aa.find("<title>")+len("<title>")
                                ending = aa.find("</title>")
                                da1[queue_song[str(reaction.message.guild.id)][re[3][str(reaction.message.guild.id)]]] = aa[starting:ending].replace(
                                    "&#39;", "'").replace(" - YouTube", "").replace("&amp;", "&")
                ...
                if reaction.emoji == emoji.emojize(":keycap_*:") and reaction.message.author == client.user:
                    ...
                    bitrate = ""
                    length = "\nLength of queue: " + \
                        str(len(queue_song[str(reaction.message.guild.id)]))
                    if reaction.message.guild.voice_client != None:
                        bitrate = "\nBitrate of the channel: " + \
                            str(reaction.message.guild.voice_client.channel.bitrate//1000)
                    if str(user) != str(client.user) and reaction.message.author == client.user:
                        st = ""
                        await reaction.remove(user)
                ...
                if str(user.id) in dev_users:
                    ...
                    if reaction.emoji == emoji.emojize(":laptop:") and str(reaction.message.channel.id) == str(channel.id) and reaction.message.author == client.user:
                        ...
                        for i in dev_users:
                            string = string + \
                                str(client.get_user(int(i)).name)+"\n"
                    ...
                    if reaction.emoji == emoji.emojize(":classical_building:") and str(reaction.message.channel.id) == str(channel.id) and reaction.message.author == client.user:
                        os.system("nohup python "+location_of_file +
                                  "/Storage.py &> .storage.txt &")
                        await reaction.remove(user)
                    ...
                    if reaction.emoji == emoji.emojize(":black_circle:") and str(reaction.message.channel.id) == str(channel.id):
                        ...
                        embed = discord.Embed(
                            title="DEVOP", description=text_dev, color=discord.Color(value=re[8]))
                        embed.set_thumbnail(
                            url=client.user.avatar_url_as(format="png"))
                        mess = await channel.send(embed=embed)
                        await mess.add_reaction(emoji.emojize(":safety_vest:"))
                        await mess.add_reaction("⭕")
                        await mess.add_reaction("❌")
    ...
    async def add_censor(ctx, *, text):
        ...
        for i in range(0, len(text)):
            string = string+"-"
        em = discord.Embed(title="Added "+string+" to the list",
                           decription="Done", color=discord.Color(value=re[8]))
        await ctx.send(embed=em)
        ...
        try:
            ...
            if f'<@!{client.user.id}>' in msg.content:
                ...
                for i in msg.author.mutual_guilds:
                    st = st+i.name+"\n"
                embed = discord.Embed(
                    title="Hi!! I am Alfred.", description="Prefix is '\nFor more help, type 'help"+st, color=discord.Color(value=re[8]))
                embed.set_image(
                    url="https://giffiles.alphacoders.com/205/205331.gif")
        ...
        if (str(text).find("username") == -1 and str(text).find("os") == -1 and str(text).find("ctx.") == -1 and str(text).find("__import__") == -1 and str(text).find("sys") == -1 and str(text).find("psutil") == -1 and str(text).find("clear") == -1 and str(text).find("dev_users") == -1 and str(text).find("remove") == -1 and str(text).find("class.") == -1 and str(text).find("subclass()") == -1 and str(text).find("client") == -1 and str(text).find("quit") == -1 and str(text).find("exit") == -1) or (str(ctx.author.id) in dev_users and str(text).find("reboot") == -1 and str(text).find("shut") == -1):
            if str(ctx.author.guild.id) != "727061931373887531":
                try:
                    ...
                    print(text)
                    em = discord.Embed(
                        title=text, description=text+"="+str(a), color=discord.Color(value=re[8]))
                    em.set_thumbnail(
    ...
    async def exe(ctx, *, text):
        ...
        if (ctx.author.id in list(temp_dev.keys()) and not (text.find("os.") != -1 and text.find("psutil") != -1 and text.find("import psutil") != -1 and text.find("import os") != -1 and text.find("__import__") != -1 and text.find("import sys") != -1 and str(text).find("subclass()") == -1 and str(text).find("client") == -1 and text.find("sys.") != -1 and text.find("subprocess.") != -1 and text.find("dev_users") != -1 and text.find("temp_dev") != -1)) or (str(ctx.author.id) in dev_users):
            ...
            with redirect_stdout(f):
                try:
                    ...
                    error_mssg = "Following Error Occured:\n" + \
                        "\n".join([line for line in traceback.format_exception(
                            type(e), e, e.__traceback__) if "in exe" not in line])
            ...
            em = discord.Embed(title="Output", description=str(
                output), color=discord.Color(value=re[8]))
            em.set_thumbnail(
                url="https://banner2.cleanpng.com/20180715/phb/kisspng-python-javascript-logo-clojure-python-logo-download-5b4ba705f356d3.4338622815316846139967.jpg")
            await ctx.send(embed=em)
        else:
    ...
    async def get_req(ctx):
        req()
        number = g_req()
        em = discord.Embed(title="Requests", description=str(
            number), color=discord.Color(value=re[8]))
        await ctx.send(embed=em)
    ...
    async def test_embed(ctx):
        embed = cembed(title="Title of the embed", description="This is to check if the embed function works",
                       thumbnail=client.user.avatar_url_as(format="png"), picture=ctx.author.avatar_url_as(format="png"), color=re[8])
        await ctx.send(embed=embed)
    ...
    async def help(ctx):
        req()
        print("help")
        em = discord.Embed(title="```Help```", description=te,
                           color=discord.Color(value=re[8]))
        em.set_thumbnail(
