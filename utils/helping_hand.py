...
FIELDS_MESSAGE = {
    ...
    "__GIF SUPPORT FOR EFFECTS__": "Earlier, if you had GIF as your profile picture, it would just return noise, now it returns a GIF, and it looks amazing\n"
    + "Although we had to decrease quality for some of the effects as it is a lot heavier to process it",
    "__NATIVE SUPPORT FOR LIST IN YML_EMBED__": [
        "You could now list out things using yaml",
        "Like this field",
        "This is created as a list",
        "Supports `description` and `field values`",
        "Syntax: \n{}".format(SAMPLE_YAML),
    ],
