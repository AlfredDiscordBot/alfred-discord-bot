...
async def fetch_trending():
    ...
    box = soup.find('div', class_="application-main").main.find("div", class_="position-relative container-lg p-responsive pt-6").div
...
class TrendDictGen:
    ...
    def process_misc_data(self):
        ...
        if lang:
            lang_color = lang.find("span", class_="repo-language-color")['style'].split()[-1]
            self.di['color'] = nextcord.Color.from_rgb(*ef.extract_color(lang_color))
            self.di['fields'].update({'Stats': f'`Language: ` {lang.get_text().strip()}\n'})
